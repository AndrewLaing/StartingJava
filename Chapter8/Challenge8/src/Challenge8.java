...
public class Challenge8
{
    ...
    private static int getLowestScoreSubscript(int StudentNumber)
    {
        int lowestScoreSubscript = 0;
        double lowestScore = StudentScores[StudentNumber][0];
        double score;

        for(int i=1; i<numberOfTests; i++)
        {
            score = StudentScores[StudentNumber][i];
            if(score<lowestScore)
            {
                lowestScore=score;
                lowestScoreSubscript=i;
            }
        }

        return lowestScoreSubscript;
    }


    // Method adjusted to skip the students lowest score
    private static double getStudentAverageScore(int StudentNumber)
    {
        double average = 0;
        int lowestScoreSubscript = getLowestScoreSubscript(StudentNumber);

        for(int i=0; i<numberOfTests; i++) {
            if(i!=lowestScoreSubscript)
                average += StudentScores[StudentNumber][i];
        }

        return average/(numberOfTests-1);
    }
